...
## The process of setting up the cluster

The setting up of the cluster involves the following **overall steps and processes**:

1. **[Loadbalancer](HAproxy_loadbalancer.md)** - which will be the entry point for trafic to services provided by the cluster
   * Install HAproxy
...
2. **[Prepare all nodes](prepare_server_for_cluster_node.md)**, master and worker nodes, which can be done relatively fast using syncronisation in `tmux`
   ...
   * Install `kubeadm`
3. [**Initialise** and **join nodes**](initialize_join_nodes.md)  with `kubeadm init` and `kubeadm join`
4. Now **just use the cluster** for what you need
